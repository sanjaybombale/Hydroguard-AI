...
def add_demo_data(user_id):
    ...
    # Add sample alerts
    alert_messages = [
        ("daily_exceeded", "🚨 Daily limit exceeded! Used 135.2L (Limit: 100L)", 2),
        ("daily_warning", "⚠️ Approaching daily limit! Used 85.2L (Limit: 100L)", 4),
        ("hourly_exceeded", "🚨 Hourly limit exceeded! Used 12.3L (Limit: 10L)", 1),
        ("unusual_pattern", "⚠️ Unusual usage pattern detected during night hours", 3),
        ("conservation_tip", "💡 Tip: Consider checking for leaks - usage higher than usual", 5)
    ]
    
    for alert_type, message, days_ago in alert_messages:
        alert_time = datetime.now() - timedelta(days=days_ago, hours=random.randint(1, 23))
        cursor.execute('''
            INSERT INTO alerts (user_id, alert_type, message, timestamp)
            VALUES (?, ?, ?, ?)
        ''', (user_id, alert_type, message, alert_time))
...
def export_user_data_to_csv(user_id):
    ...
    conn.close()
    return daily_df, None, None, None

def generate_ml_predictions(user_id):
    """Generate accurate ML-based water usage predictions using Random Forest with scaling"""
    try:
        daily_df, _, _, _ = export_user_data_to_csv(user_id)

        if len(daily_df) < 5:
            return None, "Insufficient data for predictions (need at least 5 days)"
        ...
        daily_df['prev_usage'] = daily_df['total_usage'].shift(1)
        daily_df['rolling_avg_7'] = daily_df['total_usage'].rolling(window=7, min_periods=1).mean()

        # Drop rows with NaN values (due to lag)
        daily_df.dropna(inplace=True)

        # Select features and target
        feature_cols = ['day_number', 'day_of_week', 'is_weekend', 'prev_usage', 'rolling_avg_7']
        X = daily_df[feature_cols]
        y = daily_df['total_usage']
        ...
        # Prepare to predict next 5 days
        last_date = daily_df['date'].max()
        last_day_number = daily_df['day_number'].max()
        last_usage = daily_df['total_usage'].iloc[-1]
        last_rolling = daily_df['rolling_avg_7'].iloc[-1]

        predictions = []

        for i in range(1, 6):
            future_date = last_date + timedelta(days=i)
            day_num = last_day_number + i
            ...
            input_raw = pd.DataFrame([{
                ...
                'day_of_week': dow,
                'is_weekend': is_wknd,
                'prev_usage': last_usage,
                'rolling_avg_7': last_rolling
            }])
            ...
            # Predict
            pred_usage = model.predict(input_scaled)[0]
            pred_usage = max(0, round(pred_usage, 1))  # No negative usage

            # Update state
            last_usage = pred_usage
            last_rolling = (last_rolling * 6 + pred_usage) / 7
        ...
        # Trend
        daily_change = round((predictions[-1]['predicted_usage'] - predictions[0]['predicted_usage']) / 4, 2)
...
def generate_prediction_chart(daily_df, predictions, user_id):
    ...
    try:
        ...
        plt.plot(dates, daily_df['total_usage'], 
                ...
                label='Actual Usage', color='#0d6efd', alpha=0.8)
        ...
        plt.plot(pred_dates, pred_values, 
                ...
                linestyle='--', label='Predicted Usage', color='#dc3545', alpha=0.8)
        
        # Formatting
        plt.title('🧠 Water Usage Forecast - Next 5 Days', fontsize=18, fontweight='bold', pad=20)
        plt.xlabel('Date', fontsize=14, fontweight='bold')
        plt.ylabel('Water Usage (Liters)', fontsize=14, fontweight='bold')
...
def get_water_usage_data(user_id, days=14):
                ...
                <div class="col-md-10">
                    <div class="card">
                        <div class="card-body text-center p-5">
                            ...
                            <h1 class="display-4 text-primary">Hydroguard AI</h1>
...
def register():
    ...
    <html>
    <head>
        <title>Register - Hydroguard AI</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
        <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
        <style>
            body { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh; }
...
def login():
    if request.method == 'POST':
        ...
        <html>
        <head>
            <title>Login - Hydroguard AI</title>
            <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
            <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
            <style>
                body { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh; }
...
def dashboard():
    ...
    <html>
    <head>
        <title>Dashboard - Hydroguard AI</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
        <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
        ...
        <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
            <div class="container">
                <a class="navbar-brand" href="/dashboard">
                    <i class="fas fa-tint text-info"></i>Hydroguard AI
                </a>
